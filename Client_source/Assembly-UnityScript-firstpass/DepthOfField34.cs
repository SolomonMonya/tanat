...
public class DepthOfField34 : PostEffectsBase
{
	...
	public override void AllocateTextures(bool blurForeground, RenderTexture source, int divider, int lowTexDivider)
	{
		...
		if (bokeh)
		{
			bokehSource = RenderTexture.GetTemporary(source.width / checked(lowTexDivider * bokehDownsample), source.height / checked(lowTexDivider * bokehDownsample), 0);
			bokehSource2 = RenderTexture.GetTemporary(source.width / checked(lowTexDivider * bokehDownsample), source.height / checked(lowTexDivider * bokehDownsample), 0);
