...
namespace UnityEngine
{
	public class GUI
	{
		...
		internal static Rect DoWindow(int id, Rect clientRect, WindowFunction func, GUIContent title, GUIStyle style, bool forceRectOnLayout)
		{
			...
			if (window == null)
			{
				window = (_Window)(_WindowList.instance.windows[id] = new _Window(id));
		...
		public static void DragWindow(Rect position)
		{
			...
			case EventType.MouseDown:
				if (position.Contains(current2.mousePosition) && GUIUtility.hotControl == 0)
				{
					GUIUtility.hotControl = controlID;
					Event.current.Use();
					Matrix4x4 matrix = _Window.current.matrix;
					windowDragState.dragStartPos = GUIClip.GetAbsoluteMousePosition() - (Vector2)matrix.MultiplyPoint(new Vector2(current.rect.x, current.rect.y));
		...
		internal static void BeginWindows(Event e, int skinMode, IDList idlist, int editorWindowInstanceID)
		{
			...
			case EventType.MouseDown:
			{
				...
				foreach (_Window sortedWindow in GetSortedWindowList())
				{
					GUI.matrix = sortedWindow.matrix;
					if (sortedWindow.rect.Contains(current.mousePosition))
					{
