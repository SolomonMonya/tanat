			...
			if (isVertical)
			{
				foreach (GUILayoutEntry entry in entries)
				{
					...
					if (entry.style != GUILayoutUtility.spaceStyle)
					{
						...
						childMinWidth = Mathf.Max(entry.minWidth + (float)margin.horizontal, childMinWidth);
						childMaxWidth = Mathf.Max(entry.maxWidth + (float)margin.horizontal, childMaxWidth);
					}
