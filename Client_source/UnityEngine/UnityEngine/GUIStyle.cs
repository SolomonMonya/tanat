...
namespace UnityEngine
{
	...
	public sealed class GUIStyle
	{
		...
		public static GUIStyle none => s_None;
		...
		private static void Internal_Draw(IntPtr target, Rect position, string text, Texture image, bool isHover, bool isActive, bool on, bool hasKeyboardFocus)
		{
			...
			Internal_Draw(ref arguments, isHover, isActive, on, hasKeyboardFocus);
		}

		[MethodImpl(MethodImplOptions.InternalCall)]
		[WrapperlessIcall]
		private static extern void Internal_Draw(ref Internal_DrawArguments arguments, bool isHover, bool isActive, bool on, bool hasKeyboardFocus);
		...
		public void Draw(Rect position, GUIContent content, int controlID, bool on)
		{
			...
			{
				if (flag2 || flag3 || GUIUtility.hotControl == controlID)
				{
					GUI.s_EditorTooltip = (GUI.s_MouseTooltip = content.tooltip);
					Vector2 vector = GUIUtility.GUIToScreenPoint(new Vector2(position.x, position.y));
					GUI.s_ToolTipRect = new Rect(vector.x, vector.y, position.width, position.height);
		...
		[MethodImpl(MethodImplOptions.InternalCall)]
		[WrapperlessIcall]
		private static extern void Internal_DrawWithTextSelection(ref Internal_DrawWithTextSelectionArguments arguments, bool isHover, bool isActive, bool on, bool hasKeyboardFocus, bool drawSelectionAsComposition);
		...
		internal void DrawWithTextSelection(Rect position, GUIContent content, int controlID, int firstSelectedCharacter, int lastSelectedCharacter, bool drawSelectionAsComposition)
		{
			...
			Internal_DrawWithTextSelection(ref arguments, position.Contains(current.mousePosition), controlID == GUIUtility.hotControl, on: false, controlID == GUIUtility.keyboardControl && showKeyboardFocus, drawSelectionAsComposition);
		}
