...
namespace UnityEngine
{
	public class GUIUtility
	{
		...
		internal static Vector2 textFieldCursorPos
		{
			get
			{
				INTERNAL_get_textFieldCursorPos(out var value);
				return value;
			}
			set
			{
				INTERNAL_set_textFieldCursorPos(ref value);
			}
		}
		...
		internal static void BeginGUI(Event e, int skinMode, int instanceID, int useGUILayout, IDList idlist)
		{
			s_IsInOnGUI = true;
			s_SkinMode = skinMode;
			...
			GUI.ResetSettings();
			GUIClip.Begin(e, e.type);
			SelectIDList(idlist, instanceID, isWindow: false, Event.current.type == EventType.Layout, resetIDListCursor: true);
			if (useGUILayout != 0)
			{
		...
		protected static void MoveNextAndScroll(bool forward)
		{
			...
			MoveKeyboardFocus(GUIUtility.keyboardControl, forward, sendToNextScriptIfNeeded: true);
			if (keyboardControl != GUIUtility.keyboardControl && keyboardRects.ContainsKey(GUIUtility.keyboardControl) && activeScrollView != null)
			{
				activeScrollView.ScrollTo((Rect)keyboardRects[GUIUtility.keyboardControl]);
			}
		}
		...
		[MethodImpl(MethodImplOptions.InternalCall)]
		[WrapperlessIcall]
		private static extern void INTERNAL_get_textFieldCursorPos(out Vector2 value);

		[MethodImpl(MethodImplOptions.InternalCall)]
		[WrapperlessIcall]
		private static extern void INTERNAL_set_textFieldCursorPos(ref Vector2 value);
	}
}
