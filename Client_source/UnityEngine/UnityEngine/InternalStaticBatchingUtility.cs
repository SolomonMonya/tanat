...
namespace UnityEngine
{
	internal class InternalStaticBatchingUtility
	{
		...
		public static void Combine(GameObject staticBatchRoot, bool combineOnlyStatic, bool generateTriangleStrips)
		{
			...
			{
				if ((!(staticBatchRoot != null) || gameObject.transform.IsChildOf(staticBatchRoot.transform)) && (!combineOnlyStatic || gameObject.isStatic))
				{
		...
		public static void Combine(GameObject[] gos, GameObject staticBatchRoot, bool generateTriangleStrips)
		{
			...
			foreach (GameObject gameObject in gos)
			{
				...
				if (!(meshFilter == null) && !(meshFilter.sharedMesh == null) && !(meshFilter.renderer == null) && meshFilter.renderer.enabled && meshFilter.renderer.staticBatchIndex == 0)
				{
					...
					Material[] sharedMaterials = meshFilter.renderer.sharedMaterials;
					if (sharedMaterials.Length > meshInstance.mesh.subMeshCount)
					{
						Debug.LogWarning("Mesh has more materials (" + sharedMaterials.Length + ") than subsets (" + meshInstance.mesh.subMeshCount + ")");
					}
					for (int j = 0; j < Math.Min(sharedMaterials.Length, meshInstance.mesh.subMeshCount); j++)
					{
						...
						subMeshInstance.subMeshIndex = j;
		...
		private static void MakeBatch(ArrayList meshes, ArrayList subsets, Transform staticBatchRootTransform, int batchIndex, bool generateTriangleStrips)
		{
			if (meshes.Count >= 2)
			{
				MeshSubsetCombineUtility.MeshInstance[] combines = (MeshSubsetCombineUtility.MeshInstance[])meshes.ToArray(typeof(MeshSubsetCombineUtility.MeshInstance));
				...
				Mesh combinedMesh = MeshSubsetCombineUtility.CombineVertices(combines, text);
				MeshSubsetCombineUtility.CombineIndices(array, generateTriangleStrips, ref combinedMesh);
				int num = 0;
