...
namespace UnityEngine
{
	...
	public sealed class Terrain : MonoBehaviour
	{
		...
		internal bool m_DrawTreesAndFoliage = true;
		...
		{
			foreach (Terrain ms_ActiveTerrain2 in ms_ActiveTerrains)
			{
				ms_ActiveTerrain2.SetLightmapIndex(lightmapIndex);
			}
		}
		...
		private static void CullAllTerrainsShadowCaster(Light light)
		{
			foreach (Terrain ms_ActiveTerrain2 in ms_ActiveTerrains)
			{
				Renderer renderer = ms_ActiveTerrain2.GetRenderer();
				if (renderer != null && (ms_ActiveTerrain2.m_EditorRenderFlags & TerrainRenderFlags.trees) != 0 && ms_ActiveTerrain2.m_DrawTreesAndFoliage && (double)ms_ActiveTerrain2.m_TreeDistance > 0.001)
				{
					renderer.trees.RenderShadowCasters(light, renderer.camera, Mathf.Min(ms_ActiveTerrain2.m_TreeBillboardDistance, ms_ActiveTerrain2.m_TreeDistance), ms_ActiveTerrain2.m_TreeMaximumFullLODCount, ms_ActiveTerrain2.gameObject.layer);
				}
			}
		}
