...
namespace UnityEngine
{
	public class TextEditor : IKeyboardControl
	{
		...
		public void Insert(char c)
		{
			...
			ReplaceSelection(new string(value));
		}
		...
		public void DrawCursor(string text)
		{
			...
			{
				content.text = text.Substring(0, pos) + GUIUtility.compositionString + text.Substring(selectPos);
			}
			else
			{
				content.text = text;
			...
			GUIUtility.textFieldCursorPos = graphicalCursorPos + new Vector2(position.x, position.y + style.lineHeight);
			...
			if (GUIUtility.compositionString.Length > 0)
			{
				style.DrawWithTextSelection(position, content, controlID, pos, pos + GUIUtility.compositionString.Length, drawSelectionAsComposition: true);
			}
			else
			{
